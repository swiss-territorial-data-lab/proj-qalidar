...
elif verify_out_folder(OUTPUT_DIR):
...
if RUN_ON_FOLDER:
    ...
    prev_tiles_list = [file for file in os.listdir(PREV_FOLDER_DIR) if 'copc' not in file]
else: # Run on a single tile
    print(f'Starting change detection process for tile: {PREV_TILE_NAME}\n')
    prev_tiles_list = [PREV_TILE_NAME]

new_tiles_list = [file for file in os.listdir(NEW_FOLDER_DIR) if 'copc' not in file]
...
for prev_tile in prev_tiles_list:
    ...
    if len(matching_new_tiles)==0:
        ...
        print(BColors.WARNING + 
              f'Did not find matching new tile in folder for {prev_tile}. Make sure the tiles share the same name. Note that the file format can be las or laz.\nSkipping to next tile.'
              + BColors.ENDC)
        continue
    ...
    tic = time.time()    
    voxelised_df = voxelisation.main(WORKING_DIR, prev_tile_path, new_tile_path, CLASSES_CORRESPONDENCE_PATH, VOX_SIZE)

    print(f'{tile_counter}/{total_nb_tiles}: Voxelised tile {tile_name}. ({round(time.time()-tic, 2)} sec)')

    tic = time.time()    
    criticality_df = decision_tree.main(voxelised_df, cfg, VOX_SIZE)
    
    print(f'{tile_counter}/{total_nb_tiles}: Ran decision tree on tile {tile_name}. ({round(time.time()-tic, 2)} sec)')
    
    tic = time.time()    
    clustered_df = dbscan.main(criticality_df, cfg, VOX_SIZE)

    print(f'{tile_counter}/{total_nb_tiles}: Ran DBSCAN clustering on tile {tile_name}. ({round(time.time()-tic, 2)} sec)')

    tic = time.time()
    visualisation.main(saving_dir, clustered_df, cfg, tile_name, VOX_SIZE)
...
print()
print(f'Finished entire change detection process in: {round(time.time()-start_time, 2)} sec.')
